...
そこで、XCUITest + Python Pillowを使って、撮影から加工まで完全自動化するパイプラインを作りました。この記事では、個人開発でも実践できる自動化の仕組みを詳しく解説します。
...
**相対日付の威力**
`Date()` から相対的に日付を生成することで、撮影日に関係なく「生きた」画面になります。絶対日付だと「2024年の記録」などが表示され、古臭く見えてしまいます。

**DispatchQueue.main.asyncAfter の理由**
データ注入直後に画面遷移すると、描画が間に合わないことがありました。1秒待つことで、安定して目的の画面をキャプチャできるようになります。
...
「マイクの使用を許可しますか？」などのシステムアラートは厄介です。これが出ると撮影が止まってしまうので、`addUIInterruptionMonitor` で自動応答させることで回避します。

私は最初これを知らず、手動でアラートを消していました...
...
### Step 3: 画像を「救出」する（抽出スクリプト）

標準の `XCTAttachment` を使うと、画像は `.xcresult` バンドルの中に埋もれてしまいます。ここから画像を救出するPythonスクリプトを書きます。
...
### Step 4: 「映える」画像に加工する（Python + Pillow）

ここが本記事の最大のこだわりポイントです。生のスクリーンショットに、デバイスフレームとキャッチコピーを合成して、ストア映えする画像に仕上げます。Python の `Pillow` ライブラリを使えば、コードだけで完結します。

#### 設計思想：YAML駆動で変更に強く

デザインの定義はすべてYAMLに外出しします。こうすることで、文言修正や色変更のたびにPythonコードを触る必要がなくなります。
...
ここが私の最大のこだわりです。複数のスクリーンショットを横に並べた時に、背景が自然に繋がる「パノラマ効果」を実現しました。

私のアプリは「声」がテーマなので、音波（波形）を背景に描いています。これを画像素材で用意しようとすると、端末サイズごとに調整が必要で大変です。しかし、**sin関数を使ってプログラムで描画**することで、どんなサイズでも自動対応できるようになります。
...
### 🎨 こだわりポイント2: 無いなら描く「プログラマブル・ベゼル」

最新のiPhoneが発表されるたび、デバイスフレームの画像素材を探し回るのは本当に大変です。そこで、**素材が見つからなければコードで描画する**というフォールバック機能を実装しました。
...
私は最初、iPhone 15 Pro用の素材を丁寧に作り込んでいました。しかし、iPhone 16が発表されると素材が見つからず、作業が完全にストップしてしまったんです。

このフォールバック機能があれば、**素材がなくても作業を止めずに進められます**。Pillowの `rounded_rectangle` で描くシンプルな枠だけでも、意外と様になるものです。特にダークモードのスクリーンショットなら、シンプルな枠の方がコンテンツが映えることもあります。

「完璧な素材がないから諦める」のではなく「80点でもいいから進む」という姿勢が、個人開発では本当に大切だと実感しています。



### 🎨 こだわりポイント3: YAML駆動で変更に強く

エンジニアあるあるですが、コピーライティングは何度も変わります。

- 「"声の日記"より"音声日記"の方がいいかも」
- 「サブタイトル要らないかも」
- 「文字色をもう少し濃くしたい」

こういう変更のたびにPythonコードを編集するのは本当に辛いです。YAMLに外出ししておけば、**非エンジニアでも気軽に変更できる**ようになります。
